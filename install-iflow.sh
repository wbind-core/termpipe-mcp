...
echo "🚀 Installing TermPipe MCP for iFlow CLI..."
echo ""
...
if jq -e '.mcpServers.termpipe' "$SETTINGS_FILE" > /dev/null 2>&1; then
    echo "⚠️  TermPipe MCP already configured in iFlow CLI"
...
if [ "$SKIP_CONFIG" != "true" ]; then
    echo "📝 Updating iFlow settings.json..."
...
# Add TermPipe documentation to IFLOW.md
echo ""
echo "📚 Adding TermPipe documentation to IFLOW.md..."

# Check if already documented
if [ -f "$IFLOW_MD" ] && grep -q "TermPipe MCP Tools" "$IFLOW_MD" 2>/dev/null; then
    echo "   ⚠️  TermPipe documentation already exists in IFLOW.md"
...
EOF
    echo "   ✅ Documentation added to $IFLOW_MD"
fi
...
echo "2. Restart iFlow CLI or start a new session"
echo "3. iFlow will automatically have access to TermPipe tools"
echo ""
echo "Test it by asking iFlow:"
